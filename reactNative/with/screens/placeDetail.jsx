...
function PlaceDetail({route}) {
    ...
    const changeHandle = ()=>{
        navigation.navigate("placeChange",{idToken:ctx.auth.data.idToken,data:feed})     
    }
    ...
    <View style={globalStyles.detailContainer}> 
        ...
        <View style={{flexDirection:"row", justifyContent:"space-between", marginVertical:12}}>
            <FeedFontEng style={{fontSize:12}}>by. {place.email}</FeedFontEng>
            <FeedFontEng style={{fontSize:12}}>{date.toISOString().slice(0,10)}</FeedFontEng>
        </View>
        <FeedFontKor style={{fontSize:36,marginTop:24}}>{place.contents}</FeedFontKor>
        <MapView />
